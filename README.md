...
We ran all experiments on a Linux cluster with CUDA 12.4, using [poetry](https://github.com/python-poetry/poetry) to manage dependencies. All dependencies are listed in the pyproject.toml.

To prepare the environment, run:
```
cd /path/to/rom-visde
poetry install
```
And add the root directory to the PYTHONPATH:
```
export PYTHONPATH=$PYTHONPATH:$(pwd)
```
Validate the installation by running a training test:
```
poetry run python tests/test_sde.py
```
This should take a few minutes to run and produce a figure `tests/test_sde.png` with a plot of a noisy exponential function.

Each experiment is contained in a subdirectory of `experiments/`. The structure is as follows:
```
experiments
...
│   │   └───train_psindy.py
│   ├───sindy_champion
│   │   └───// copy of Champion et al.'s repo (see paper)
│   └───visde // our method
│       ├───def_model.py
│       ├───make_animation.py
│       ├───postproc_visde.py
...
│       └───train_visde.py
├───forced_burgers_1d
│   └───// subdirectories with different methods
└───flow_control_2d
    └───// subdirectories with different methods
```
The relevant code for our methodology is in the `visde/` subdirectory of each experiment. The script `train_visde.py` is for training, `postproc_visde.py` will calculate prediction error and generate results figures for trained models, and `def_model.py` contains the model architecture. The last script `run_visde.py` is a simple wrapper that will run `train_visde.py` followed by `postproc_visde.py` for the same model. This pattern is consistent across all experiments, including for the baseline methods that we implemented.
