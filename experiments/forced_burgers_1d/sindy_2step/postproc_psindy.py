...
from experiments.forced_burgers_1d.sindy_2step.def_model import create_sindy_model, create_autoencoder, TestAutoencoder
...
def main(data_file: str = "data_bcf_100_10_10.pkl",
    ...
    for i in range(n_traj):
        ...
        if compression == "pod":
            x_sim_i = np.matmul(z_sim_i, modes.T)
        elif compression == "ae":
            with torch.no_grad():
        ...
        tsamples = [0, 250, 500, 750, 1000]
        n_tstep_sim = x_sim_i.shape[0]
        
        fig, axs = plt.subplots(len(tsamples), 1, figsize=(8, 4*len(tsamples)))
        for j, tj in enumerate(tsamples):
            if tj >= n_tstep_sim:
                break
            axs[j].plot(x_i[tj, :], label="True")
            axs[j].plot(x_sim_i[tj, :], label="Pred")
            axs[j].legend()
        fig.show()
