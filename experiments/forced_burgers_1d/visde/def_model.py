...
class DecodeMeanNet(nn.Module):
    def __init__(self, config):
        ...
        self.deconvnet = nn.Sequential(nn.ConvTranspose1d(fac*16, fac*8, kern, stride=2, padding=kern//2, output_padding=1),
                                       nn.ReLU(),
                                       nn.ConvTranspose1d(fac*8, fac*4, kern, stride=2, padding=kern//2),
                                       nn.ReLU(),
                                       nn.ConvTranspose1d(fac*4, fac*2, kern, stride=2, padding=kern//2),
                                       nn.ReLU(),
...
class DecodeVarNet(nn.Module): # for node/nsde only
    def __init__(self, config):
    ...
    def forward(self, mu: Tensor, z: Tensor) -> Tensor:
        x_var_norm = self.fixed_var.expand(z.shape[0], *self.fixed_var.shape[1:])
        x_var = self.out_activ(x_var_norm)
        return x_var
...
class DriftNetNODE(nn.Module):
    def __init__(self, config):
        super(DriftNetNODE, self).__init__()
        dim_mu = config.dim_mu
        dim_z = config.dim_z

        self.net = nn.Sequential(nn.Linear(dim_mu + dim_z + 2, 128),
                                 nn.ReLU(),
                                 nn.Linear(128, 128),
                                 nn.ReLU(),
                                 nn.Linear(128, 128),
    ...
    def forward(self, mu: Tensor, t: Tensor, z: Tensor, f: Tensor) -> Tensor:
        mu_z_t = torch.cat([mu, z, torch.cos(t), torch.sin(t)], dim=-1)
        dz = self.net(mu_z_t)
        return dz
