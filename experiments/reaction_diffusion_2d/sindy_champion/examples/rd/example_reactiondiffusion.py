import numpy as np
from datasets.reaction_diffusion_2d.load_data import load_mat_data


def get_rd_data(random=True):
    data = load_mat_data('reaction_diffusion.mat')

    n_samples = data['t'].size
    n = data['x'].size
    N = n*n

    data['uf'] += 1e-2*np.random.randn(data['uf'].shape[0], data['uf'].shape[1], data['uf'].shape[2])
    data['duf'] += 1e-2*np.random.randn(data['duf'].shape[0], data['duf'].shape[1], data['duf'].shape[2])
