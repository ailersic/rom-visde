...
def main(overwrite: bool = False,
        data_file: str = "data_noisy.pkl",
        dim_z = 2,
        n_epochs: int = 1000,
        lr: float = 3e-3,
        lr_sched_freq: int = 1000,
) -> None:
    n_win = 1
    n_batch_train = 128
    n_batch_val = 256
    ...
    model = create_latent_sde(dim_z, n_batch_train, n_win, data_file, lr, lr_sched_freq, device)

    version = "_".join([data_file.split(".")[0], str(dim_z), str(n_batch_train), str(n_batch_val), str(n_epochs), str(lr), str(lr_sched_freq)])
