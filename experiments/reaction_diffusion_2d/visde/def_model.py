...
class DecodeVarNet(nn.Module):
    def __init__(self, config):
        ...
        self.out_activ = nn.Softplus()
        self.fixed_var = nn.Parameter(-4.0*torch.ones((1, *shape_x)))

    def forward(self, mu: Tensor, z: Tensor) -> Tensor:
        z_var_norm = self.fixed_var.expand(z.shape[0], *self.fixed_var.shape[1:])
        z_var = self.out_activ(z_var_norm)
        return z_var

'''
class PolynomialDriftWithLaplacePrior(nn.Module):
    ...
    def __init__(
        ...
        self.n_coeffs = self.dim_z * 10

        self.register_buffer('prior_loc', torch.zeros(self.n_coeffs).to(self.device))
        self.register_buffer('prior_logscale', -4.0*torch.ones(self.n_coeffs).to(self.device))
        self.poly_loc = nn.Parameter(torch.clone(self.prior_loc).to(self.device))
        self.poly_logscale = nn.Parameter(torch.clone(self.prior_logscale).to(self.device))
    ...
    def resample_params(self) -> None:
        stdlap_samples = torch.distributions.Laplace(0, 1).sample(self.prior_loc.shape).to(self.device)
        self.poly_coeffs = self.poly_loc + torch.mul(stdlap_samples, torch.exp(self.poly_logscale))
    
    def kl_divergence(self) -> Tensor:
        prior_loc = self.prior_loc
        prior_scale = torch.exp(self.prior_logscale)

        var_loc = self.poly_loc
        var_scale = torch.exp(self.poly_logscale)

        loc_diff = torch.abs(var_loc - prior_loc)

        kl_div = torch.sum(torch.log(prior_scale/var_scale)
                           + (var_scale*torch.exp(-loc_diff/prior_scale) + loc_diff)/prior_scale
                           - 1)
        return kl_div
    ...
    def forward(self,
        ...
        dzdt = torch.einsum('ij,jk->ik', z_poly3, self.poly_coeffs.reshape(10, self.dim_z)) #torch.sum(z_poly3 * self.poly_coeffs.reshape(10, self.dim_z), dim=(1,))
        return dzdt
'''
...
class VarDecoderWithLogNormalPrior(nn.Module):
    ...
    def __init__(
        ...
        self.dec_logvar_prior_mean = torch.full(self.shape_x, -4.0).to(self.device)
...
def create_latent_sde(dim_z: int,
    ...
    #drift = PolynomialDriftWithLaplacePrior(driftconfig)
