...
class DriftNet(nn.Module):
    def __init__(self, config):
        ...
        self.net = nn.Sequential(nn.Linear(dim_z + dim_f, 512),
                                 nn.LeakyReLU(),
                                 nn.Linear(512, 512),
                                 nn.LeakyReLU(),
                                 nn.Linear(512, dim_z))
...
class DriftNetNODE(nn.Module):
    def __init__(self, config):
        ...
        self.net = nn.Sequential(nn.Linear(dim_z + dim_mu, 512),
                                 nn.LeakyReLU(),
                                 nn.Linear(512, 512),
                                 nn.LeakyReLU(),
                                 nn.Linear(512, dim_z))
...
def create_latent_sde(dim_z: int,
    ...
    #data_mean, data_std = torch.mean(data["train_x"], dim=(0,1)), torch.std(data["train_x"], dim=(0,1))
    ...
    kernel_net = KernelNet(kernelconfig)
