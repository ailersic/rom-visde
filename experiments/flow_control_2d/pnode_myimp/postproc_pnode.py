...
import visde
from experiments.flow_control_2d.visde.def_model import DriftNet, EncodeMeanNet, DecodeMeanNet
from experiments.flow_control_2d.pnode_myimp.train_pnode import create_autoencoder, create_pnode, PNODEConfig, TestPNODE, PNODE
...
def plot_pred_state(traj_dir, x_true, x_mean, x_std, dim_x, tsamples, t):
    for chan in range(x_true.shape[1]):
        figpred = plt.figure(figsize=(16, 2*len(tsamples)))
        ...
        for j, j_t in enumerate(tsamples):
            ...
            axgrid[id1].imshow(x_true[j_t, chan])
            ...
            axgrid[id2].imshow(x_mean[j_t, chan])
            ...
            axgrid[id3].imshow(np.abs(x_mean[j_t, chan] - x_true[j_t, chan]))
            ...
            axgrid[id4].imshow(x_std[j_t, chan])
        ...
        figpred.savefig(os.path.join(traj_dir, f"pred_vs_true_chan_{chan}.pdf"), format='pdf')
        figpred.show()
        plt.close(figpred)
...
def plot_error_dist(out_dir, rel_err, t_plot):
    ...
    ax.boxplot(rel_err[:, periods], positions=t_plot[periods], widths=0.1, showfliers=False)
    ...
    figrmse.savefig(os.path.join(out_dir, "rel_err_dist.pdf"), format='pdf')
    figrmse.show()
    plt.close(figrmse)

def main(data_file: str = "data_param.pkl", dim_z = 9):
    ...
    version = "_".join([data_file.split(".")[0], str(dim_z), "133", "200"])
    ckpt_dir = os.path.join(CURR_DIR, "logs_pnode", version, "checkpoints")
    out_dir = os.path.join(CURR_DIR, "postproc_pnode", version)
    ...
    model = TestPNODE.load_from_checkpoint(os.path.join(ckpt_dir, ckpt_file),
                                            config=pnodeconfig,
                                            encoder=encoder,
                                            decoder=decoder,
                                            drift=drift
    ...
    for i_traj in range(n_traj):
        ...
        print(f"Integrating PNODE for trajectory {TRAIN_VAL_TEST} {i_traj}...", flush=True, end="")
        ...
        z0_i = model.encoder.sample(n_batch, mu_i, x0_i).mean(0, keepdim=True)
        pnode = PNODE(model.drift, mu_i)
        with torch.no_grad():
            z_int = odeint(pnode, z0_i, t_i)
        ...
        for j_t in range(n_tsteps):
            ...
            with torch.no_grad():
                x_mean[j_t] = model.decoder.sample(n_batch_decoder, mu_i, z_int[j_t]).mean(0)
