...
from experiments.flow_control_2d.visde.def_model import EncodeMeanNet, EncodeVarNet, DecodeMeanNet, DecodeVarNet
...
def create_sindy_model(x, dx, u, dt, threshold, degree):
    ...
    model = ps.SINDy(
        feature_library=feature_lib,
        optimizer=opt,
        feature_names=[f"x{i}" for i in range(dim_x)] + [f"p{i}" for i in range(dim_u)],
        discrete_time=False,
    )

    model.fit(x, u=u, t=dt, x_dot=dx, multiple_trajectories=True)
