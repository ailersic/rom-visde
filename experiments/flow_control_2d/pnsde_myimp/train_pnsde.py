...
from experiments.flow_control_2d.visde.def_model import DriftNetNODE, DispNet, EncodeMeanNet, DecodeMeanNet, EncodeVarNet, DecodeVarNet
...
class TestPNSDE(pl.LightningModule):
    ...
    drift: visde.LatentDriftNoPrior
    dispersion: visde.LatentDispersionNoPrior
    ...
    def training_step(self,
        ...
        z_int = sdeint(temp_sde, z_enc_0, t, method="euler").squeeze(1)
    ...
    def validation_step(self,
        ...
        z_int = sdeint(temp_sde, z_enc_0, t).squeeze(1)
        x_pred_mean, x_pred_var = self.decoder(mu, z_int)
...
def create_autoencoder(dim_z: int, n_win: int, data_file: str) -> tuple[nn.Module, nn.Module]:
    ...
    decode_var_net = DecodeVarNet(vaeconfig, data_mean, data_std)
    decoder = visde.VarDecoderNoPrior(vaeconfig, decode_mean_net, decode_var_net)

    return encoder, decoder

def create_pnsde(dim_z: int, data_file: str) -> nn.Module:
    data = load_data(data_file)
    ...
    driftnet = DriftNetNODE(driftconfig)
...
def main(overwrite: bool = False, data_file: str = "data_param.pkl", dim_z = 9):
    n_win = 1
    n_batch = 133
    ...
    version = "_".join([data_file.split(".")[0], str(dim_z), str(n_batch), "200"])
    '''
    if os.path.exists(os.path.join(CURR_DIR, "logs_pnsde", version)):
        if overwrite:
            print(f"Version {version} already exists. Overwriting...", flush=True)
            while os.path.exists(os.path.join(CURR_DIR, "logs_pnsde", version)):
                try:
                    shutil.rmtree(os.path.join(CURR_DIR, "logs_pnsde", version))
                except OSError as e:
                    continue
        else:
    ...
    tensorboard = loggers.TensorBoardLogger(CURR_DIR, name="logs_pnsde", version=version)
    ...
    pnsde_model = TestPNSDE(config=pnsdeconfig,
                            encoder=encoder,
                            decoder=decoder,
                            drift=drift,
                            dispersion=dispersion).to(device)
    
    pnsde_trainer = pl.Trainer(
    ...
    pnsde_trainer.fit(pnsde_model, train_dataloader, val_dataloader, ckpt_path=os.path.join(ckpt_dir, ckpt_file))
